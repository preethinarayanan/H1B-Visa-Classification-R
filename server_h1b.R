...
#Split out DV and predictors

y <- h1bdrop$CASE_STATUS #keep your DV away from all processing and filtering to avoid overfitting!
x <- h1B[,1:24] #only first 16 columns (predictors)
str(x)

#model them as dummy variables
#head(model.matrix(CASE_STATUS ~ . - EMPLOYER_NAME - WORKSITE_POSTAL_CODE, data = h1B))
head(model.matrix( ~ . , data = x))
...
library(caret)
#h1bdrop$CASE_STATUS <- as.factor(h1bdrop$CASE_STATUS)
str(h1bdrop$CASE_STATUS)
#dummies <- dummyVars( ~., data = h1bdrop)
h1bdrop$CASE_STATUS <- relevel(h1bdrop$CASE_STATUS, ref="CERTIFIED")
dummies <- dummyVars( ~., data = h1bdrop)
...
dummies.h1b <- predict(dummies, newdata = h1bdrop)
dummies.h1b$CASE_STATUS <- h1bdrop$CASE_STATUS #copy back DV
class(dummies)
...
library(e1071)
pp_hpc1 <- preProcess(dummies.h1b[, ], 
...
transformed1 <- predict(pp_hpc1, newdata = dummies.h1b[, ])
head(transformed1)
...
nzv <- nearZeroVar(dummies.h1b, saveMetrics= TRUE) #saving metrics to view variable details
nzv[nzv$nzv,][1:25,]
#shows zero or near zero variance

dim(dummies.h1b) #342 variables to start

nzv <- nearZeroVar(dummies.h1b) #no metrics just return identified columns indexes
nzv

#remove or filter columns with nearZeroVar
#subset n say not any of these
#ncol(filteredh1b)
ncol(dummies.h1b)
filteredh1b <- dummies.h1b[, -nzv]
dim(filteredh1b) #297 columns remaining

##remove highly correlated variables

#first create correlation matrix of variables
descrCor <-  cor(filteredh1b)
...
filteredDescr <- filteredh1b[,-highlyCorDescr]

#creaet new correlation matrix to verify
descrCor2 <- cor(filteredh1b)
summary(descrCor2[upper.tri(descrCor2)]) #no correlations greater than .75
#dimensionlity affects computation

#numpending very low variance
#mean(dummies.h1bdrop$NumPending == 0) #76%=0

#feature reduction using PCA

#pp_no_pca <- preProcess(h1B[, -8, -26], method = c("pca"))
#pp_no_pca
#PCs <- prcomp(filteredh1b)
#pp_no_pca1 <- preProcess(filteredh1b[, -452:-453], method = c("pca"))
#pp_no_pca1
#varImp(pp_no_pca1)
...
#head(predict(pp_no_pca1, newdata = filteredh1b[ , ]))
#finaldata <- (predict(pp_no_pca1, newdata = filteredh1b[ , ]))

##Putting it all together
#using the AppliedPredictiveModeling package for the book
library(AppliedPredictiveModeling)
#data(dummies.h1bdrop)
#str(schedulingData)
...
#trying to predict 8 th class omit last 8 th column
#leaving out class variable
pp_hpc <- preProcess(dummies.h1b[, ], 
                     method = c("center", "scale", "BoxCox", "nzv"))
pp_hpc #can see results of processing

#now we juse need to apply the processing model to the data
transformed <- predict(pp_hpc, newdata = dummies.h1b[, ])
head(transformed)
...
pp_no_nzv <- preProcess(dummies.h1b[, ], 
                        method = c("center", "scale", "BoxCox", "nzv"))
pp_no_nzv #1 removed

#apply model to data and see new subset and processed variables
finaldata <- predict(pp_no_nzv, newdata = dummies.h1b[, ])
#numpending removed coz of zero variance
#pca and missing data
#schedMissing<-schedulingData
#lets insert 4 random variables in compound to show imputation
#schedMissing$Compounds[c(3,6,12,44)] <- c(NA,NA,NA,NA)

#lets process and impute missing values
#schedMissing$Compounds[c(3,6,12,44)]
#remove label to predict coz might change coz of processing
...
#pp_no_pca <- preProcess(dummies.x[, ], 
#method = c("pca"))
#pp_no_pca <- preProcess(pp_no_nzv[, ],
#method = c("pca"))
pp_no_pca <- preProcess(finaldata[, ], 
                        method = c("pca"))
pp_no_pca 

#apply model to data and see new subset and processed variables
data <- predict(pp_no_pca, newdata = dummies.h1b)
#NOT MUCH IMPUTATION...DATA SET NOT MUCH IMPUTATION  IN THIS COURSE
#interpret--1pc contains most inof,each subset less n less..find leas
...
View(data)
?createDataPartition
trainIndex <- createDataPartition(data$CASE_STATUS, p = .8, 
                                  list = FALSE, 
                                  times = 1)
head(trainIndex)
View(y)
#create datapartiton wats label iris..11
#cor.test(h1B$PW_SOURCE_OTHER, h1B$SOC_NAME)
h1BTrain <- data[ trainIndex,]

#create training set subset (20%)
h1BTest  <- data[-trainIndex,]
table(h1BTrain$CASE_STATUS)
table(h1BTest$CASE_STATUS)
...
#smote_train <- SMOTE(CASE_STATUS ~ ., data  = filteredh1b) 
